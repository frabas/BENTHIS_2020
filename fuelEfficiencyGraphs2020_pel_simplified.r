...
# find metiers lvl6-Vsize to merge
 ...
 # met to keep
 oth_mets <- c(oth_mets_pel, as.character(dem[,1]), 
                                "NA_No_Matrix6","No_Matrix6_[12,18)", 
                                                      paste0("NA") )
 }
 #----
  ...
  if(per_metier_level6 && !per_vessel_size && per_region) {
   ...
   aggResult$sp_with_max_vpufswa <-   gsub("LE_VPUFSWA_", "", names(aggResult[,idx_cols])[dd])          
    ...
    # capture an export for quickmap2020.r   (and for DISPLACE North Sea)
     ...
     if(per_metier_level6 && !per_vessel_size && !per_region) {
       save(aggResult, file=file.path(getwd(), "outputs2020_pel", paste("AggregatedSweptAreaPlusMet6AndRatiosForPelAllReg_", y, ".RData", sep=""))) 
       }
 ...
 ##!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!###
  ...
  # save:
  if(per_metier_level6 && !per_vessel_size  && per_region) save(aggResultPerMetAlly, file=file.path(getwd(), "outputs2020_pel", paste("aggResultPerMetAllyMet6AndRatiosForPel.RData", sep=""))) 
  ...
  if(per_metier_level6 && !per_vessel_size && !per_region) save(aggResultPerMetAlly, file=file.path(getwd(), "outputs2020_pel", paste("aggResultPerMetAllyMet6AndRatiosForPelAllReg.RData", sep=""))) 
...
#load(file=file.path(getwd(), "outputs2020_pel", paste("aggResultPerMetAllyMet6AndVsizeAndRatiosForPel.RData", sep="")))  # aggResultPerMetAlly
if(per_metier_level6 && !per_vessel_size && per_region) load(file=file.path(getwd(), "outputs2020_pel", paste("aggResultPerMetAllyMet6AndRatiosForPel.RData", sep="")))  # aggResultPerMetAlly
if(per_metier_level6 && !per_vessel_size && !per_region) load(file=file.path(getwd(), "outputs2020_pel", paste("aggResultPerMetAllyMet6AndRatiosForPelAllReg.RData", sep="")))  # aggResultPerMetAlly
spp <- colnames(aggResultPerMetAlly) [grep("LE_EURO_", colnames(aggResultPerMetAlly))]   ; spp <- gsub("LE_EURO_", "", spp) ; spp <- spp [!spp=="SPECS"]
...
# PEL
 ...
 p3_barplot_pelfishing_pel_cpuf <- ggplot(data=the_agg_plot3, aes(x=met_desc, y=value/a_unit, fill=Stockname)) + #  geom_bar(stat="identity", position=position_dodge())
  geom_bar(stat = "summary", fun = a_func_mean) +  labs(y ="Landings (kg) per litre fuel", x= "Vessels (>12m) using pelagic gears") +
 ...
 #print(p3_barplot_pelfishing_pel_cpuf)
