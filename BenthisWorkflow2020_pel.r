...
load(file=file.path(outPath,a_year,"tacsatMerged.RData"))
  ...
  fm        <- list(TBB=0.13,
                    ...
                    OTM=0.13, # DNK close to straight line (fm at 0)
                    OTT=0.13,
                    PTB=0.13,
                    PTM=0.13,
                    PS=0.13,
...
#} # end a_year
 ...
 for (a_year in years){
    ...
    some_color_vessel_size <- c("[12,18)"="#FFDB6D",  "[18,24)"="#FC4E07",  "[24,40)"="#52854C",  "[40,100)"="#293352")
     ...
     p <- ggplot() +  ggplot() + geom_bar(data=aggEffortAlly, aes(x=as.character(Year), y=effective_effort_mins/60, group=VesselSize, fill=VesselSize), size=1.5, position="stack",  stat = "summary", fun = "sum") +
