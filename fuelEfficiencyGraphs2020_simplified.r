...
# independent workflow for passive gears (just using (Danish) eflalo files)
 ...
 if(per_metier_level6 && !per_vessel_size && !per_region){
   ...
   for (y in years){
      ...
      dd <- tapply(aggResult$effort_mins, paste0(aggResult$target, "_", aggResult$LE_MET_init), sum)
      res <- rbind.data.frame(res, cbind.data.frame(names(dd), dd))
      }
   ...
   # but keep two exeption anyway to get some representation for SSC and GNS 
   #LargeMesh_27.3_SSC_DEF_>=120_0_0 
   oth_mets_dem <- oth_mets_dem[!oth_mets_dem %in% c('LargeMesh_SSC_DEF_>=120_0_0')]
 
 # met to keep
 oth_mets <- c(oth_mets_dem, oth_mets_pel, "No_Matrix6_[12,18)","No_Matrix6", paste0("NA","_") )
 }
 #----
 ...
 for (y in years){
   ...
   if(per_metier_level6 && !per_vessel_size && !per_region) {
      ...
      aggResult$LE_MET <- factor(paste0(aggResult$target, "_", aggResult$LE_MET))
      
      # manage the OTHER mets
      smallmesh_oth_mets <- oth_mets[grepl("SmallMesh",oth_mets)]
      largemesh_oth_mets <- oth_mets[grepl("LargeMesh",oth_mets)]
      levels(aggResult$LE_MET)[levels(aggResult$LE_MET) %in% smallmesh_oth_mets] <- "SmallMesh_OTHER_0_0"
      levels(aggResult$LE_MET)[levels(aggResult$LE_MET) %in% largemesh_oth_mets] <- "LargeMesh_OTHER_0_0"
      levels(aggResult$LE_MET)[levels(aggResult$LE_MET) %in% oth_mets[!oth_mets %in% c(smallmesh_oth_mets, largemesh_oth_mets)]] <- "LargeMesh_OTHER_0_0_0"
      levels(aggResult$LE_MET)[grepl("NA", levels(aggResult$LE_MET)) ] <- "NA_OTHER_0_0_0"
      
      # caution - a small fix for an annoying renaming of segment
      aggResult$LE_MET <- factor(aggResult$LE_MET)
      levels(aggResult$LE_MET)[levels(aggResult$LE_MET) %in% "LargeMesh_OTB_CRU_>=120_0_0"] <- "LargeMesh_OTB_DEF_>=120_0_0"
      aggResult$LE_MET <- factor(aggResult$LE_MET)
   ...
   aggResult$sp_with_max_vpufswa <-   gsub("LE_VPUFSWA_", "", names(aggResult[,idx_cols])[dd])          
    ...
    # capture an export for quickmap2020.r  (and DISPLACE NorthSea)
     ...
     if(per_metier_level6 && !per_vessel_size && !per_region) {
       save(aggResult, file=file.path(getwd(), "outputs2020", paste("AggregatedSweptAreaPlusMet6AndRatiosForBottContactAllReg_", y, ".RData", sep=""))) 
     }
 ...
 ##!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!###
  ...
  if(per_metier_level6 && !per_vessel_size  && per_region) save(aggResultPerMetAlly, file=file.path(getwd(), "outputs2020", paste("aggResultPerMetAllyMet6AndRatiosBottContactAndGNS.RData", sep=""))) 
  ...
  if(per_metier_level6 && !per_vessel_size && !per_region) save(aggResultPerMetAlly, file=file.path(getwd(), "outputs2020", paste("aggResultPerMetAllyMet6AndRatiosBottContactAndGNSAllReg.RData", sep=""))) 
 ...
 #load(file=file.path(getwd(), "outputs2020", paste("aggResultPerMetAllyMet6AndVsizeAndRatiosBottContactAndGNS.RData", sep="")))  # aggResultPerMetAlly
  ...
  aggResultPerMetAlly$FPUE <- aggResultPerMetAlly$LE_KG_LITRE_FUEL / (aggResultPerMetAlly$effort_mins/60)
  ...
  ### plot FPUE
  library(ggplot2)
  dd <- aggResultPerMetAlly[,c("Year", "FPUE", "LE_MET", "LE_MESH_GROUP")]
  ...
  dd$met_desc <- friendly_met_names(dd)   # the ones used for color seg
  
  a_fpue_plot <- ggplot(dd, aes(x=as.character(Year), y=as.numeric(FPUE), group=as.character(met_desc))) +     
                             geom_line(aes(color=met_desc), size=1.5) + 
  ...
  namefile <- paste0("ts_FPUE_", years[1], "-", years[length(years)],  "_DEM_PEL_rev.tif")
  tiff(filename=file.path(getwd(), "outputs2020", "output_plots",  namefile),   width = a_width, height = a_height,
                                   units = "px", pointsize = 12,  res=600, compression = c("lzw"))
  library(ggpubr)
  ggarrange(a_fpue_plot, ncol=1, common.legend = TRUE, legend="right")
...
#REDO SOME STUFF FOR A STANDALONE CODE:
 ...
 #load(file=file.path(getwd(), "outputs2020", paste("aggResultPerMetAllyMet6AndVsizeAndRatiosBottContact.RData", sep="")))  # aggResultPerMetAlly
 #load(file=file.path(getwd(), "outputs2020", paste("aggResultPerMetAllyMet6AndRatiosBottContact.RData", sep="")))  # aggResultPerMetAlly
 if(per_metier_level6 && !per_vessel_size && per_region) load(file=file.path(getwd(), "outputs2020", paste("aggResultPerMetAllyMet6AndRatiosBottContactAndGNS.RData", sep="")))  # aggResultPerMetAlly
 if(per_metier_level6 && !per_vessel_size && !per_region) load(file=file.path(getwd(), "outputs2020", paste("aggResultPerMetAllyMet6AndRatiosBottContactAndGNSAllReg.RData", sep="")))  # aggResultPerMetAlly
...
# DEM
 ...
 the_agg_plot$LE_MET <- gsub("LargeMesh_", "", the_agg_plot$LE_MET)
  ...
  ss <- the_agg_plot[grepl ( "(a)",  the_agg_plot$LE_MET)   & as.numeric(as.character(the_agg_plot$value))>20, ]  # < 20tons?
...
the_agg_plot1 <- as.data.frame(the_agg_plot[grep("(a)",the_agg_plot$LE_MET, fixed=TRUE),])
 ...
 p3_barplot_bottomfishing_dem_cpuf <- ggplot(data=the_agg_plot3, aes(x=met_desc, y=value/a_unit, fill=Stockname)) + #  geom_bar(stat="identity", position=position_dodge())
  geom_bar(stat = "summary", fun = a_func_mean) +
    labs(y ="Landings (kg) per litre fuel", x= "Vessels (>12m) using bottom-contacting gears with large meshes") +
       scale_fill_manual(values=some_color_speciesnames[unique(the_agg_plot3$Stockname)], name="Species") + theme_minimal() + theme(axis.text.x=element_blank()) + guides(fill =guide_legend(ncol=1, position="right"))+  
        # theme(axis.text.x=element_blank()) 
         theme(axis.text.x=element_text(angle=60,hjust=1,vjust=1, size=12))
  #print(p3_barplot_bottomfishing_dem_cpuf)
...
##!!!!!!!!!!!!!!!!!!!!!!##
##!!!!!!!!!!!!!!!!!!!!!!##
## BOTTOM TRAWL WITH SMALL MESHES
#REDO SOME STUFF FOR A STANDALONE CODE:
 setwd (file.path("D:","FBA","BENTHIS_2020"))   # adapt to your need
 library(doBy)
 years <- 2005:2019
 ...
 for (y in years){
  ...
  the_agg <- the_agg[!grepl("misc.",the_agg$met_desc),] # get rid of OTHER
  the_agg <- the_agg[!grepl("Gillnet",the_agg$met_desc),] # get rid of non representative gillnet
...
# PEL
 ...
 the_agg_plot3$LE_MET <- factor(the_agg_plot3$LE_MET, level=fleet_segments_ordered) # reorder
 the_agg_plot3$met_desc <- factor(the_agg_plot3$met_desc, level=fleet_segments_ordered2) # reorder)
  p3_barplot_bottomfishing_pel_cpuf <- ggplot(data=the_agg_plot3, aes(x=met_desc, y=value/a_unit, fill=Stockname)) + #  geom_bar(stat="identity", position=position_dodge())
  #geom_bar(stat = "summary", fun = a_func_mean) +
  #  labs(y ="CPUF (kg per litre)", x= "") +
  #     scale_fill_manual(values=some_color_species, name="Species")  + theme_minimal() + theme(axis.text.x=element_blank()) + guides(fill =guide_legend(ncol=1, position="right"))  
 geom_bar(stat = "summary", fun = a_func_mean) +
    labs(y ="Landings (kg) per litre fuel", x= "Vessels (>12m) using bottom-contacting gears with no or small meshes") +
         scale_fill_manual(values=some_color_speciesnames[unique(the_agg_plot3$Stockname)], name="Species") + theme_minimal() + theme(axis.text.x=element_blank()) + guides(fill =guide_legend(ncol=1, position="right"))+  
        # theme(axis.text.x=element_blank()) 
         theme(axis.text.x=element_text(angle=60,hjust=1,vjust=1, size=12))
  #print(p3_barplot_bottomfishing_pel_cpuf)
 ...
 the_agg_plot5 <- as.data.frame(the_agg_plot[grep("(e)",the_agg_plot$LE_MET, fixed=TRUE),])
  ...
  geom_bar(stat = "summary", fun = a_func_mean) + 
   labs(y = "Litre per kg catch", x= "Fleet-segments") +
         scale_fill_manual(values=some_color_speciesnames[unique(the_agg_plot3$Stockname)], name="Species") + theme_minimal() + guides(fill =guide_legend(ncol=1, position="right"))  + 
        # theme(axis.text.x=element_blank()) 
         theme(axis.text.x=element_text(angle=60,hjust=1,vjust=1, size=12))
...
setwd (file.path("D:","FBA","BENTHIS_2020"))   # adapt to your need
 ...
 for (y in years){
  ...
  # a check 
  #ff <- the_agg[grepl("(a)",as.character(the_agg$LE_MET), fixed=TRUE) & the_agg$Stock=="COD 27.3",]
  #sum(tapply(ff$value, ff$met_desc, mean))
  #ff <- the_agg[grepl("(a)",as.character(the_agg$LE_MET), fixed=TRUE) & the_agg$Stock=="PLE 27.3",]
  #sum(tapply(ff$value, ff$met_desc, mean))
...
# DEM
 the_agg_plot <- as.data.frame(the_agg[grep("LargeMesh",the_agg$LE_MET),])
   ...
   # select some species
  the_agg_plot <- the_agg_plot[the_agg_plot$Stock %in% c("COD 27.3", "COD 27.4",  "HAD 27.4", "MON 27.4", "FLE 27.3", "LEM 27.4", "WIT 27.3", "HKE 27.4", "DAB 27.4", "NEP 27.3", "PLE 27.3", "PLE 27.4", "POK 27.4", "SOL 27.3"),]
 ...
 the_agg_plot$LE_MET <- gsub("LargeMesh_", "", the_agg_plot$LE_MET)
  ...
  # filter out small contribution metier 
  ss <- the_agg_plot[grepl ( "(a)",  the_agg_plot$LE_MET)   & as.numeric(as.character(the_agg_plot$value))>200, ]  # < 20tons?
  the_agg_plot <- the_agg_plot[the_agg_plot$met_desc %in% unique(ss$met_desc),]
 ...
 the_agg_plot1$LE_MET <- gsub("\\(a)","", the_agg_plot1$LE_MET)
  ...
  p1_barplot_bottomfishing_dem_land_per_stk <- ggplot(data=the_agg_plot1, aes(x=Stock, y=value/1e3, fill=met_desc)) + #  geom_bar(stat="identity", position=position_dodge())
 ...
 p2_barplot_bottomfishing_dem_fuel_per_stk <- ggplot(data=the_agg_plot2, aes(x=Stock, y=value/1e6, fill=met_desc)) + #  geom_bar(stat="identity", position=position_dodge())
...
#------------
 ...
 the_agg_plot1$LE_MET <- gsub("\\(a)","", the_agg_plot1$LE_MET)
  the_agg_plot1$Stock <- factor(the_agg_plot1$Stock, level=stock_ordered) # reorder
  p1_barplot_bottomfishing_pel_land_per_stk <- ggplot(data=the_agg_plot1, aes(x=Stock, y=value/1e3, fill=met_desc)) + #  geom_bar(stat="identity", position=position_dodge())
  geom_bar(stat = "summary", fun = "mean") +  labs(y = "Landings (*1000 tons)", x= "") +
       scale_fill_manual(values=some_color_seg, name="Fleet-segments") + theme_minimal() + theme(axis.text.x=element_blank()) + guides(fill =guide_legend(ncol=1, position="right"))
  print(p1)
 ...
 the_agg_plot2$LE_MET <- gsub("\\(b)","", the_agg_plot2$LE_MET)
  the_agg_plot2$Stock <- factor(the_agg_plot2$Stock, level=stock_ordered) # reorder
 p2_barplot_bottomfishing_pel_fuel_per_stk <- ggplot(data=the_agg_plot2, aes(x=Stock, y=value/1e6, fill=met_desc)) + #  geom_bar(stat="identity", position=position_dodge())
  geom_bar(stat = "summary", fun = "mean") +  labs(y ="Fuel (millions litre)", x= "") +
       scale_fill_manual(values=some_color_seg, name="Fleet-segments")  + theme_minimal() + theme(axis.text.x=element_blank()) + guides(fill =guide_legend(ncol=1, position="right"))  
  #print(p2)
...
the_agg_plot5 <- as.data.frame(the_agg_plot[grep("(e)",the_agg_plot$LE_MET, fixed=TRUE),])
  the_agg_plot5$LE_MET <- gsub("\\(e)","", the_agg_plot5$LE_MET)
   the_agg_plot5$Stock <- factor(the_agg_plot5$Stock, level=stock_ordered) # reorder
 p5_barplot_bottomfishing_pel_fpuc_per_stk <- ggplot(data=the_agg_plot5, aes(x=Stock, y=value/a_unit, fill=met_desc, order=as.numeric(met_desc))) + #  geom_bar(stat="identity", position=position_dodge())
    geom_bar(stat = "summary", fun = "mean") +  labs(y = "Litre per kg catch", x= "Species") +
       scale_fill_manual(values=some_color_seg, name="Fleet-segments") + theme_minimal() + guides(fill =guide_legend(ncol=1, position="right"))  + 
        theme(axis.text.x=element_text(angle=60,hjust=1,vjust=1, size=12))
