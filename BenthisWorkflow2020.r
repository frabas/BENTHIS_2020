...
if(.Platform$OS.type == "unix") {
 ...
 outPath   <- file.path("/zhome","fe","8","43283","BENTHIS", "outputs2020")
   ...
   tacsatp$LE_MET         <- as.character(tacsatp$LE_MET)
    ...
    if(a_year=="2011"){
     ...
     if(a_year=="2012") {
      ...
      if(a_year=="2016"){
        levels (tacsatp$LE_MET) <- c(
          "DRB_MOL", "NA", "NA", "OT_CRU", "OT_CRU", "OT_DMF_PEL",  "OT_DMF",  "OT_DMF_PEL", "OT_DMF_PEL", "OT_DMF",
           "OT_MIX_NEP", "OT_MIX_NEP", "OT_MIX_NEP", "OT_MIX_NEP", "OT_SPF", "OT_SPF", "OT_SPF", "OT_SPF", "OT_DMF_PEL",  "OT_DMF",    
          "OT_MIX_NEP", "OT_MIX_NEP", "OT_SPF", "OT_SPF", "OT_SPF", "SDN_DEM", "SDN_DEM", "SDN_DEM", "SDN_DEM",  "SDN_DEM", "SSC_DEM", "SSC_DEM", "TBB_CRU",  "TBB_DMF" )
     
   } else{
      if(a_year=="2017"){
        levels (tacsatp$LE_MET) <- c(
         "DRB_MOL", "OT_MIX_NEP","OT_CRU", "OT_MIX_NEP", "OT_MIX_NEP", "OT_MIX_NEP","OT_DMF_PEL","OT_DMF","OT_DMF","OT_DMF",
         "OT_DMF_PEL",  "OT_DMF_PEL", "OT_DMF", "OT_DMF", "OT_DMF", "OT_DMF",  "OT_SPF", "OT_DMF_PEL",   "OT_DMF", "OT_MIX_NEP", 
         "OT_DMF_PEL", "OT_DMF", "OT_SPF", "OT_SPF", "OT_SPF",  "SDN_DEM", "SDN_DEM", "SDN_DEM", "SDN_DEM",  "SDN_DEM", 
         "SSC_DEM", "SSC_DEM", "TBB_CRU", "TBB_DMF", "TBB_DMF")
     
   } else{
      if(a_year=="2018"){
        levels (tacsatp$LE_MET) <- c(
           "DRB_MOL",     "OT_MIX_NEP",   "OT_CRU",   "OT_CRU",   "OT_CRU",   "OT_MIX_NEP",  "OT_DMF_PEL",     "OT_DMF", "OT_DMF",   "OT_DMF",
           "OT_DMF_PEL",   "OT_DMF_PEL",   "OT_DMF",   "OT_DMF",  "OT_DMF",  "OT_SPF",   "OT_SPF",  "OT_MIX_NEP",  "OT_DEM_PEL",     "OT_DMF",
           "OT_DMF", "OT_DMF", "OT_SPF", "SDN_DEM", "SDN_DEM", "SDN_DEM", "SDN_DEM",  "SSC_DEM",   "TBB_CRU",   "TBB_DMF"  
        )
   } else{
      if(a_year=="2019"){
        levels (tacsatp$LE_MET) <- c(
         "DRB_MOL",     "OT_MIX_NEP",   "OT_CRU",   "OT_MIX_NEP",  "OT_MIX_NEP",   "OT_MIX_NEP",  "OT_DEM_PEL",     "OT_DMF", "OT_DMF",   "OT_DMF",
          "OT_DMF_PEL",   "OT_DMF_PEL",   "OT_DMF",   "OT_DMF",  "OT_DMF",  "OT_SPF",   "OT_MIX_NEP",  "OT_MIX_NEP",     "OT_DMF",   "SDN_DEM",
          "SDN_DEM",   "SDN_DEM", "SDN_DEM",  "SSC_DEM",   "SSC_DEM", "SSC_DEM",  "TBB_CRU",   "TBB_CRU",   "TBB_DMF",   "TBB_DMF"
      )
      }else{
  ...
  for(iGr in towedGears){
    ...
    for(iVE_REF in sort(unique(tacsatpGear$VE_REF))){
      ...
      if(nrow(tacsatpGearVEREF)>3){
       ...
       do_it <-TRUE
       if(length(fls)!=0 && length(grep(paste("tacsatSweptArea_",iVE_REF, "_", iGr, ".RData", sep=""),fls)!=0)  && overwrite==FALSE) do_it <- FALSE
       ...
       if(do_it) er <- try({
        ...
        cat(paste(iGr, " ", iVE_REF, "...done\n"))
         }, silent=TRUE)
        if(class(er)=="try-error") print('error for this interpolation')
      }
    }
  }
...
#- Combine tacsatSweptArea files over years to obtain aggregated swept areas per grid cell (caution, not SAR yet)
for(iYr in years){
#- read data for this year
load(file.path(outPath, iYr, paste("tacsatSweptArea.RData", sep="")))
...
xrange  <- c(-7,25) # do not change to be able to make the overlay over countries
yrange  <- c(50,65)  # do not change to be able to make the overlay over countries
...
#replaced by:
grid_degrees             <- 0.01  # less than 1 minute
tacsatSweptArea$c_square <- vmstools::CSquare (lon=tacsatSweptArea[,"SI_LONG"], lat=tacsatSweptArea[,"SI_LATI"], degrees=grid_degrees)


# 1. Aggregate the results by metier and grid ID (here: c_square) (aggregate() can be slow: be patient)
aggTacsatSweptArea <- aggregate(tacsatSweptArea[,c("SWEPT_AREA_KM2",
"SWEPT_AREA_KM2_LOWER",
"SWEPT_AREA_KM2_UPPER")],
by=list(tacsatSweptArea$LE_MET,tacsatSweptArea$c_square,tacsatSweptArea$SI_YEAR),sum,na.rm=T)
colnames(aggTacsatSweptArea)[1:3] <- c("LE_MET","c_square", "Year")
...
#CELL_LATI=coordinates(grd)[aggTacsatSweptArea$c_square,2])
aggResult          <- cbind.data.frame(aggTacsatSweptArea, CSquare2LonLat(aggTacsatSweptArea$c_square, grid_degrees)) # get the mid point coordinates
save(aggResult,file=file.path(outPath,paste0("AggregatedSweptArea_",range(years)[1],"-",range(years)[2],".RData")))
...
by=list(tacsatSweptArea$LE_MET,tacsatSweptArea$MONTH,tacsatSweptArea$c_square,tacsatSweptArea$SI_YEAR),sum,na.rm=T)
colnames(aggTacsatSweptArea2)[1:4] <- c("LE_MET","MONTH", "c_square", "Year")

#  Add midpoint of gridcell to dataset
#aggResult <- cbind(aggTacsatSweptArea2,CELL_LONG=coordinates(grd)[aggTacsatSweptArea2$c_square,1],
#CELL_LATI=coordinates(grd)[aggTacsatSweptArea2$c_square,2])
aggResult          <- cbind.data.frame(aggTacsatSweptArea2, CSquare2LonLat(aggTacsatSweptArea2$c_square, grid_degrees)) # get the mid point coordinates
save(aggResult,file=file.path(outPath,paste0("AggregatedSweptAreaMonth_",range(years)[1],"-",range(years)[2],".RData")))
...
"SWEPT_AREA_KM2_LOWER", "SWEPT_AREA_KM2_UPPER")],
by=list(tacsatSweptArea$LE_MET, tacsatSweptArea$DAY, tacsatSweptArea$c_square,tacsatSweptArea$SI_YEAR),sum,na.rm=T)
colnames(aggTacsatSweptArea3)[1:4] <- c("LE_MET","DAY", "c_square", "Year")

#- Add midpoint of gridcell to dataset
#aggResult <- cbind(aggTacsatSweptArea3, CELL_LONG=coordinates(grd)[aggTacsatSweptArea3$c_square,1],
#CELL_LATI=coordinates(grd)[aggTacsatSweptArea3$c_square,2])
aggResult          <- cbind.data.frame(aggTacsatSweptArea3, CSquare2LonLat(aggTacsatSweptArea3$c_square, grid_degrees)) # get the mid point coordinates
save(aggResult,file=file.path(outPath,paste0("AggregatedSweptAreaDay_",range(years)[1],"-",range(years)[2],".RData")))
...
save(aggResult,file=file.path(outPath,paste0("missingEffortTable_",years[1],"-",years[2],".RData", sep='')))
